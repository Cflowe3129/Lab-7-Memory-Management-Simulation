...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  while(current != NULL) {
    if(current->blk->end - current->blk->start >= blocksize) { //Check if a node is in the FREE_LIST with a blk(end - start) >= blocksize
      block_node = current;
      break; //remove node and go to 3
    }
  ...
  block_node->blk->pid = pid; //set the blk.pid = pid
  block_node->blk->end = block_node->blk->start + blocksize - 1; //set the blk.end = blk->start + blocksize - 1
  list_add_ascending_by_address(alloclist, block_node->blk); //add the blk to the ALLOC_LIST in ascending order by address.
  
  if(block_node->blk->end != blk_end) {
    block_t *fragment = (block_t*)malloc(sizeof(block_t));
    
    fragment->pid = 0; //set the fragment->pid = 0 
    fragment->start = block_node->blk->end + 1; //set the fragment->start = the blk.end + 1
    fragment->end = blk_end; //set the fragment->end = original blk.end
    
    if(policy == 1) {
      list_add_to_back(freelist, fragment); //first fit
    } else if(policy == 2) {
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
   ...
   block_t *deallocate_block_node = NULL;
   node_t *current = alloclist->head;
   node_t *prev = NULL;
  
  while(current != NULL && current->blk->pid != pid) {
    if(current->blk->pid == pid) { //Check if a node is in the ALLOC_LIST with a blk.pid = pid
      deallocate_block_node = current;
      break; //remove node and go to 3
    }
  ...
  if(prev == NULL) {
    ...
    deallocate_block_node = current->blk;
  }
  
  deallocate_block_node->pid = 0; //set the blk.pid back to 0
  
  if(policy == 1) {
      list_add_to_back(freelist, deallocate_block_node); //first fit
    } else if(policy == 2) {
       list_add_ascending_by_blocksize(freelist, deallocate_block_node); //best fit
    } else if(policy == 3) {
      list_add_descending_by_blocksize(freelist, deallocate_block_node); //worst fit
    }
}
