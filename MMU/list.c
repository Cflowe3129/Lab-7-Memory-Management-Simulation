...
void list_add_ascending_by_address(list_t *l, block_t *newblk)
{
  ...
  { //list is not empty
    ...
    { //at the end of the list
      ...
      { //address of the new block is less than the current block's address
        newNode->next = l->head;
        l->head = newNode;
      }
      else
      { //address of the new block is greater than the  current's block's address
        current->next = newNode;
        newNode->next = NULL;
      }
    }
    else
    {
      ...
      else
      {
        ...
        { //traverse list until a block with an address less than the address of newNode is found
          prev = current;
          current = current->next;
        }
        prev->next = newNode;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk)
{
  ...
  { //list is not empty
    prev = current = l->head;
    current_blk_size = current->blk->end - current->blk->start + 1;
    ...
    if (current->next == NULL)
    {
      ...
      { //size of new block is not greater than the current block
        current->next = newNode;
        newNode->next = NULL;
      }
    }
      ...
      else
      {
        ...
        { //find block that is larger than the current block size and insert it in the list
          ...
          { //calculate current block size
            current_blk_size = current->blk->end - current->blk->start;
          }
        }
...
void list_coalese_nodes(list_t *l)
{
  ...
  { //list is not empty
    ...
    {                                     //compare two nodes at a time to see if the prev.END + 1 == current.START
      prev->blk->end = current->blk->end; //combine them by setting the prev.END = current.END.
